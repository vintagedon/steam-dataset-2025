# =================================================================================================
# File:          04-validate-materialization-full-dataset.py
...
#
# Purpose:
#   Full-dataset validation of materialized PC requirements; certifies ETL accuracy.
#
# Section Map:
...
# --- Orchestration --------------------------------------------------------------------------------
# Human: Fetch entire set; re-parse source; compare vs mat_*; emit aggregate mismatch stats.
...
def run_validation():
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            if df.empty:
                logging.warning("No rows to validate.")
                return
            ...
            logging.info(f"[FULL DATASET] Rows: {total:,} | Minimum OK: {min_ok} ({min_ok/total:.1%}) | Recommended OK: {rec_ok} ({rec_ok/total:.1%})")
    except Exception as e:
        logging.critical(f"An error occurred during full-dataset validation: {e}")
        sys.exit(1)
